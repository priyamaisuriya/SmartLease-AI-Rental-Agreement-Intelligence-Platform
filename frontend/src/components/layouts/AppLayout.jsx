...
const NAV_ITEMS = [
  ...
  { id: '/properties', label: 'Find Properties', icon: Search, section: 'Find & Rent' },
  { id: '/requests', label: 'My Rental Requests', icon: HelpCircle, badge: '2', section: 'Find & Rent' },
  { id: '/rentals', label: 'My Rentals', icon: Home, section: 'Find & Rent' },
  { id: '/agreements', label: 'My Agreements', icon: FileText, section: 'Agreements' },
  { id: '/upload', label: 'Analyze Agreement', icon: Upload, section: 'Agreements' },
  { id: '/analysis', label: 'AI Analysis', icon: FileSearch, section: 'Agreements' },
  { id: '/chat', label: 'AI Chat', icon: MessageSquare, section: 'Agreements' },
  { id: '/reminders', label: 'Reminders', icon: Bell, section: 'Agreements' },
  { id: '/notifications', label: 'Notifications', icon: Bell, badge: '4', section: 'Account' },
  { id: '/reports', label: 'Reports', icon: FileBarChart, section: 'Account' },
      ...
      <aside className="sticky top-0 hidden h-screen w-[248px] shrink-0 flex-col bg-gradient-to-b from-ink-dark to-[#101a2e] lg:flex px-[18px] py-[28px]">
            ...
            <div key={sIdx} className="mb-[22px]">
              ...
              <div className="space-y-[2px]">
                {NAV_ITEMS.filter(item => item.section === section).map(item => (
                  <NavLink key={item.id} to={item.id} className={({ isActive }) => `flex items-center gap-[11px] px-3 py-[9px] rounded-[4px] text-[13.5px] font-medium transition-colors w-full ${isActive ? 'bg-gold/12 text-gold-soft font-semibold' : 'text-ink-faint hover:bg-white/5 hover:text-[#EDEBE3]'}`}>
        ...
        <div className="mt-auto pt-4 border-t border-ink-line flex items-center gap-2.5 px-2 mt-4">
          ...
          <div className="flex-1 overflow-hidden">
            <div className="text-[13px] font-semibold text-[#EDEBE3] truncate">Priya Sharma</div>
            <div className="text-[11px] text-ink-muted block">Tenant</div>
          </div>
        ...
        <nav className="scroll-thin flex-1 overflow-y-auto">
          {sections.map((section, sIdx) => (
            <div key={sIdx} className="mb-[22px]">
              ...
              <div className="space-y-[2px]">
                {NAV_ITEMS.filter(item => item.section === section).map(item => (
                  <NavLink key={item.id} to={item.id} onClick={() => setMobileMenuOpen(false)} className={({ isActive }) => `flex items-center gap-[11px] px-3 py-[9px] rounded-[4px] text-[13.5px] font-medium transition-colors w-full ${isActive ? 'bg-gold/12 text-gold-soft font-semibold' : 'text-ink-faint hover:bg-white/5 hover:text-[#EDEBE3]'}`}>
                    <item.icon className="w-4 h-4 shrink-0" strokeWidth={1.6} />
                    {item.label}
                    {item.badge && (
                      <span className="ml-auto text-[10.5px] bg-[#C1443C] text-white px-[6px] py-[1px] rounded-[10px] font-semibold">
