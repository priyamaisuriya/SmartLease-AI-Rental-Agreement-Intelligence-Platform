    ...
    <div className="fade-in">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">Every agreement you've uploaded, linked or independent.</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
        {agreementsData.map(a => (
          <div key={a.id} className="rounded-xl2 border border-border bg-white p-5 shadow-soft">
            ...
            <p className="mt-1 text-xs text-text-faint">{a.property}</p>
            <p className="mt-1 text-xs text-text-faint">Uploaded {a.uploadDate}</p>
            ...
            <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-4">
              <button onClick={() => navigate('/analysis')} className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-ink hover:bg-paper">View</button>
              <button onClick={() => navigate('/analysis')} className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-ink hover:bg-paper">Analyze</button>
              <button onClick={() => navigate('/chat')} className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-ink hover:bg-paper">Chat with AI</button>
              <button onClick={() => alert('Downloading report…')} className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-ink hover:bg-paper">Download</button>
            </div>
          </div>
        ))}
      </div>
