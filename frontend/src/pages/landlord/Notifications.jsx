    ...
    <div className="space-y-6 fade-in pb-8 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-muted mt-1">Stay updated on your properties and agreements.</p>
        </div>
      ...
      <div className="bg-white border border-border rounded-xl shadow-sm divide-y divide-border">
        {mockNotifications.map(notif => (
          <div key={notif.id} className={`p-6 flex items-start gap-4 transition-colors ${!notif.read ? 'bg-lease-50/30' : 'hover:bg-paper/50'}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${!notif.read ? 'bg-lease-100 text-lease-600' : 'bg-paper text-text-muted border border-border'}`}>
            ...
            {!notif.read && (
              <button className="p-2 text-text-faint hover:text-lease-600 transition-colors" title="Mark as read">
