      ...
      <div className="bg-white rounded-xl shadow-sm border border-border overflow-hidden">
        ...
        <div className="px-6 sm:px-10 pb-8 relative">
          <div className="flex flex-col sm:flex-row gap-6 -mt-12 sm:-mt-16 mb-8">
            <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-surface bg-paper flex items-center justify-center text-4xl font-bold text-lease-600 flex-shrink-0">
              {getInitial()}
            </div>
            <div className="pt-2 sm:pt-16 flex-1">
              {isEditing ? (
                ...
                <>
                  <h2 className="text-2xl font-bold text-ink">{user?.name || 'Landlord User'}</h2>
                  <p className="text-text-muted font-medium capitalize">{user?.role || 'Landlord'}</p>
                </>
              )}
            </div>
          ...
          {!isEditing && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <h3 className="font-semibold text-ink border-b border-border pb-2">Contact Information</h3>
                <div className="space-y-4">
                  <div className="flex items-start gap-3 text-sm">
                    <Mail className="w-5 h-5 text-text-muted flex-shrink-0" />
                    <div>
                      <p className="text-text-faint">Email Address</p>
                      <p className="font-medium text-ink mt-0.5">{user?.email || 'Not provided'}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3 text-sm">
              ...
              {/* <div className="space-y-6">
                ...
                <div className="flex items-start gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-ink mt-0.5 text-text-muted italic">
