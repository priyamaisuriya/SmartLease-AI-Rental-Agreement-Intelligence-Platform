    ...
    { 
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <Link to={`/landlord/tenants/${row.id}`} className="p-1 text-text-muted hover:text-lease-600 transition-colors" title="View Details">
            <Eye className="w-4 h-4" />
          </Link>
          <button className="p-1 text-text-muted hover:text-good-600 transition-colors" title="Call">
            <Phone className="w-4 h-4" />
          </button>
          <button className="p-1 text-text-muted hover:text-signal-600 transition-colors" title="Email">
    ...
    <div className="space-y-6 fade-in pb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-muted mt-1">Manage all active and past tenants across your properties.</p>
        </div>
      </div>
