    ...
    <div className="fade-in">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">Cedar Heights, Pal · Analyzed 2 Sep 2026</p>
        </div>
      ...
      <div className="mt-6">
        ...
        <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {financialTerms.map((f, i) => (
            <div key={i} className="rounded-xl2 border border-border bg-white p-4">
              <p className="text-xs text-text-faint">{f.label}</p>
        ...
        <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {importantClauses.map((c, i) => (
            <div key={i} className="rounded-xl2 border border-border bg-white p-4">
              ...
              <p className="mt-1.5 text-xs leading-relaxed text-text-muted">{c.detail}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6">
        ...
        <div className="mt-3 space-y-4">
          {risks.map((r, i) => (
            <div key={i} className="rounded-xl2 border border-border bg-white p-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="font-display text-sm font-semibold text-ink">{r.title}</p>
                {riskBadge(r.level)}
              </div>
              <p className="mt-2 text-sm text-text-muted">{r.explanation}</p>
              <p className="mt-2 text-xs text-text-faint"><span className="font-medium text-ink">Why it matters:</span> {r.why}</p>
              <p className="mt-2 rounded-lg bg-paper px-3 py-2 text-xs text-text-muted"><span className="font-medium text-ink">Suggested action:</span> {r.action}</p>
            </div>
          ))}
        </div>
      </div>
