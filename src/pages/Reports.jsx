...
const Reports = () => {
  ...
  return (
    <div className="fade-in">
      ...
      <p className="mt-1 text-sm text-ink-soft">Download and review your rental and agreement history.</p>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="rounded-xl2 border border-line bg-surface p-6">
          ...
          <p className="mt-2 text-sm text-ink-soft">1 active rental, 0 past rentals recorded.</p>
        </div>
        <div className="rounded-xl2 border border-line bg-surface p-6">
          <p className="font-display text-sm font-semibold text-ink">Request history</p>
          <p className="mt-2 text-sm text-ink-soft">3 requests sent · 1 accepted, 1 pending, 1 rejected.</p>
        </div>
        <div className="rounded-xl2 border border-line bg-surface p-6">
          <p className="font-display text-sm font-semibold text-ink">Agreement analysis reports</p>
          <p className="mt-2 text-sm text-ink-soft">2 reports generated across your agreements.</p>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {reports.map(r => (
          <div key={r.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl2 border border-line bg-surface p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-ink-faint">{r.agreement} · Generated {r.date}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              {riskBadge(r.riskLevel)}
              <button onClick={() => alert(`Downloading ${r.name}`)} className="rounded-lg border border-line px-3 py-2 text-xs font-medium text-ink hover:bg-canvas">
                Download
              </button>
            </div>
          </div>
