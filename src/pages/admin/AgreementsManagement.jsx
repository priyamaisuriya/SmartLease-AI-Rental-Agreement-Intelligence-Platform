...
const AgreementsManagement = () => {
  ...
  const columns = [
    ...
    { 
      ...
      accessor: 'property',
      render: (row) => (
        <span className={row.property === 'Not Linked' ? 'text-ink-faint italic' : ''}>
          {row.property}
        </span>
      )
    },
    ...
    { 
      header: 'Risk Level', 
      accessor: 'risk',
      render: (row) => row.risk ? <StatusBadge status={`${row.risk} Risk`} /> : <span className="text-ink-faint">-</span>
    },
    { 
      header: 'Actions', 
      accessor: 'actions',
      render: (row) => (
        <div className="flex items-center gap-2">
          ...
          <button className="p-1 text-ink-soft hover:text-lease-600 transition-colors" title="Download Report">
            <Download className="w-4 h-4" />
          </button>
          <button className="p-1 text-ink-soft hover:text-bad-600 transition-colors" title="Remove">
    ...
    <div className="space-y-6 fade-in pb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-ink-soft mt-1">Monitor uploaded agreements and AI analysis status.</p>
        </div>
      </div>
