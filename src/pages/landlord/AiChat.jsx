  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto fade-in">
      ...
      <div className="flex items-center gap-4 mb-4 flex-shrink-0">
        {agreementId ? (
          <Link to={`/landlord/analysis/${agreementId}`} className="p-2 text-ink-soft hover:bg-surface rounded-lg border border-transparent hover:border-line transition-all">
            <ArrowLeft className="w-5 h-5" />
          </Link>
        ) : (
          <Link to={`/landlord`} className="p-2 text-ink-soft hover:bg-surface rounded-lg border border-transparent hover:border-line transition-all">
            <ArrowLeft className="w-5 h-5" />
          </Link>
        )}
        <div>
          ...
          <p className="text-xs text-ink-soft mt-0.5">AI-generated information is for assistance and should not be considered legal advice.</p>
        </div>
      </div>

      {/* Chat Area */}
      <div className="flex-1 bg-surface border border-line rounded-t-xl shadow-sm flex flex-col overflow-hidden">
            ...
            <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[80%] rounded-2xl p-4 text-sm shadow-sm ${
                msg.role === 'user' 
                  ...
                  : 'bg-canvas border border-line text-ink rounded-tl-sm'
              }`}>
                {msg.text}
              </div>
        ...
        {/* Suggested Questions */}
        {messages.length === 1 && (
          <div className="p-4 bg-canvas/50 border-t border-line flex flex-wrap gap-2">
            {suggestedQuestions.map((q, idx) => (
              <button 
                key={idx}
                onClick={() => handleSuggest(q)}
                className="px-3 py-1.5 bg-surface border border-line rounded-full text-xs font-medium text-ink hover:border-lease-500 hover:text-lease-600 transition-colors"
              >
                {q}
              </button>
            ))}
        ...
        <div className="p-4 border-t border-line bg-surface">
          <form onSubmit={handleSend} className="relative flex items-center">
            <input 
              ...
              className="w-full pl-4 pr-12 py-3 bg-canvas border border-line rounded-xl text-sm focus:outline-none focus:border-lease-500 focus:ring-1 focus:ring-lease-500 shadow-inner"
            />
            <button 
              type="submit"
              disabled={!input.trim()}
