    ...
    { 
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          ...
          {row.status === 'Pending' && (
            <>
              <button className="p-1 text-ink-soft hover:text-good-600 transition-colors" title="Accept Request">
                <CheckCircle className="w-4 h-4" />
              </button>
              <button className="p-1 text-ink-soft hover:text-bad-600 transition-colors" title="Reject Request">
    ...
    <div className="space-y-6 fade-in pb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-ink-soft mt-1">Review and manage incoming tenant applications.</p>
        </div>
      </div>
