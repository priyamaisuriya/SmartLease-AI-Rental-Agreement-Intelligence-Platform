  ...
  return (
    <div className="fade-in h-[calc(100vh-140px)]">
      <div className="grid h-full gap-5 lg:grid-cols-[280px_1fr]">
        ...
        <aside className="hidden flex-col rounded-xl2 border border-line bg-surface p-4 lg:flex">
          <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">Agreement</p>
          <div className="mt-2 rounded-lg bg-lease-50 p-3">
            <p className="text-sm font-medium text-ink">Cedar Heights — Lease</p>
            <p className="mt-0.5 text-xs text-ink-faint">Analyzed · Medium risk</p>
          </div>
          
          <p className="mt-5 text-xs font-medium uppercase tracking-wide text-ink-faint">Previous conversations</p>
          <div className="scroll-thin mt-2 flex-1 space-y-1.5 overflow-y-auto">
            <button className="w-full rounded-lg bg-canvas px-3 py-2.5 text-left text-sm text-ink">Notice period & deposit</button>
            <button className="w-full rounded-lg px-3 py-2.5 text-left text-sm text-ink-soft hover:bg-canvas">Rent increase clause</button>
            <button className="w-full rounded-lg px-3 py-2.5 text-left text-sm text-ink-soft hover:bg-canvas">Old Adajan agreement</button>
          </div>
        </aside>

        {/* main chat */}
        <div className="flex h-full flex-col rounded-xl2 border border-line bg-surface">
          <div className="border-b border-line px-5 py-4">
            <p className="font-display text-sm font-semibold text-ink">Ask about Cedar Heights — Lease Agreement</p>
            <p className="mt-1 text-xs text-ink-faint">AI-generated information is for assistance and should not be considered legal advice.</p>
          </div>
          ...
          <div className="scroll-thin flex-1 space-y-4 overflow-y-auto px-5 py-5">
            {messages.map((m, idx) => m.role === 'user' ? (
              ...
              <div key={idx} className="flex gap-2.5">
                ...
                <div className="max-w-[80%] rounded-xl2 rounded-tl-sm border border-line bg-canvas px-4 py-2.5 text-sm text-ink">
                  {m.text}
                </div>
              </div>
            ))}
            ...
            <div className="mb-3 flex flex-wrap gap-2">
              {chatSuggestions.map((s, idx) => (
                <button 
                  ...
                  className="rounded-full border border-line px-3 py-1.5 text-xs text-ink-soft hover:bg-canvas"
                >
                  {s}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-2 rounded-lg border border-line bg-canvas px-3 py-2.5">
              <input 
                value={inputValue}
