...
const Requests = () => {
  ...
  return (
    <div className="fade-in">
      ...
      <p className="mt-1 text-sm text-ink-soft">Track the status of properties you've requested to rent.</p>
      
      <div className="mt-6 space-y-4">
        {rentalRequests.length > 0 ? rentalRequests.map(r => (
          <div key={r.id} className="flex flex-col gap-3 rounded-xl2 border border-line bg-surface p-5 sm:flex-row sm:items-center sm:justify-between shadow-soft">
            <div className="min-w-0">
              <p className="font-display text-sm font-semibold text-ink">{r.property}</p>
              <p className="mt-1 text-xs text-ink-faint">{r.location} · Landlord: {r.landlord}</p>
              <p className="mt-1 text-xs text-ink-faint">Requested {r.requestedDate} · Listed rent ₹{r.listedRent.toLocaleString('en-IN')}/mo</p>
            </div>
            <div className="flex items-center gap-3">
              {statusBadge(r.status)}
              <Link to={`/properties/${r.propertyId}`} className="rounded-lg border border-line px-3 py-2 text-xs font-medium text-ink hover:bg-canvas">View Details</Link>
            </div>
          </div>
        )) : (
          <div className="flex flex-col items-center justify-center rounded-xl2 border border-dashed border-line bg-surface/60 px-6 py-14 text-center">
            ...
            <p className="mt-1.5 max-w-sm text-sm text-ink-faint">Browse properties and send a request to get started.</p>
          </div>
        )}
      </div>
    </div>
