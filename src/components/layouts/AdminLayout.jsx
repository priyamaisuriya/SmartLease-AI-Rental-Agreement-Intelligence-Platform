...
import { 
  LayoutDashboard, Users, Building, ClipboardList, 
  FileText, BrainCircuit, BarChart3, MessageSquare, 
  ShieldCheck, Bell, User, Settings, LogOut, Menu, X 
} from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const navigation = [
    ...
    { section: 'Management', items: [
      ...
      { name: 'Rental Requests', href: '/admin/rental-requests', icon: ClipboardList },
      { name: 'Agreements', href: '/admin/agreements', icon: FileText },
    ]},
    { section: 'AI & Reports', items: [
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-surface border-r border-line transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 flex-shrink-0 items-center justify-between px-6 border-b border-line">
          <div className="flex items-center gap-2 text-xl font-display font-bold text-ink">
            ...
            SmartLease <span className="text-lease-600">Admin</span>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 border-l border-line pl-4">
                ...
                <div className="hidden sm:block text-sm">
                  ...
                  <p className="text-ink-faint text-xs">Super Admin</p>
                </div>
              </div>
            </div>
          </div>
